...
export default function UserDetail() {
  ...
  const [rol_id, setRole] = useState('');
    ...
    useEffect(() => {
        ...
        try {
            ...
            const academic_unit_response = await fetch('http://localhost:8003/api/v1/academic_unit/get-all?skip=0&limit=50');
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8003/api/v1/user_rol_academic_unit', {
        method: 'POST',
        headers: {
  ...
  return (
    <div>
      ...
      <p><strong>Tipo de Identificación:</strong> {user.identification_type}</p>
      <p><strong>Número de Identificación:</strong> {user.identification_number}</p>
      ...
      <ul>
        {user.user_roles_academic_units.map((roleAcademicUnit, index) => (
          <li key={index}>
            {roleAcademicUnit.rol.name} - {roleAcademicUnit.academic_unit.name}
          </li>
        ))}
      </ul>
